  ...
  Future<void> _fetchAssets({bool isLoadMore = false}) async {
    ...
    try {
      if (!isLoadMore) {
        ...
        if (!ps.isAuth) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Permisos de galería denegados.'),
              ),
            );
          }
          return;
        ...
        _recentAlbum = albums.firstWhere(
          (a) => a.isAll,
          orElse: () => albums.first,
        );
      ...
      if (_recentAlbum == null || !_hasMore) return;

      final media = await _recentAlbum!.getAssetListPaged(
        page: _currentPage,
        size: _pageSize,
      );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Error cargando galería')),
        );
      }
    } finally {
      if (mounted) {
